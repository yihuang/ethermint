	...
	for {
		...
		case ev, ok := <-chHeaders:
			...
			res, err := s.validatorAccount(
				types.ContextWithHeight(data.Header.Height),
				&evmtypes.QueryValidatorAccountRequest{
					ConsAddress: sdk.ConsAddress(data.Header.ProposerAddress).String(),
				},
			)
			if err != nil {
			...
			header := types.EthHeaderFromTendermint(data.Header, ethtypes.Bloom{}, baseFee, validator)
