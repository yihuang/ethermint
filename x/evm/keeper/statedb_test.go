...
func (suite *KeeperTestSuite) TestSuicide() {
	...
	err := db.ForEachStorage(suite.address, func(key, value common.Hash) bool {
	...
	suite.Require().Equal(common.BytesToHash(types.EmptyCodeHash).Bytes(), db.GetCodeHash(suite.address).Bytes())
}
